"""Tests for user32.dll!IsCharAlphaA — previously unimplemented (hard halt)."""
...
class TestIsCharAlphaA:
    @pytest.mark.parametrize("ch", [ord("A"), ord("Z"), ord("a"), ord("z")])
    def test_true_for_alpha(self, env, ch):
        cpu, mem, state, stubs = env
        mem.write32(STACK + 4, ch)

        stubs.get("user32.dll", "IsCharAlphaA")(cpu)

        assert cpu.regs[EAX] == 1

    @pytest.mark.parametrize("ch", [ord("0"), ord("9"), ord(" "), ord("_"), 0])
    def test_false_for_digits_and_non_alpha(self, env, ch):
        cpu, mem, state, stubs = env
        mem.write32(STACK + 4, ch)
