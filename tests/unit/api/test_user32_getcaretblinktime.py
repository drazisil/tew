"""Tests for user32.dll!GetCaretBlinkTime — previously unimplemented (hard halt; hit live after the post-login CONTINUE click)."""
...
class TestGetCaretBlinkTime:
    def test_returns_530ms(self, env):
        cpu, mem, state, stubs = env

        stubs.get("user32.dll", "GetCaretBlinkTime")(cpu)

        assert cpu.regs[EAX] == 530
    ...
    def test_no_args_no_stack_cleanup(self, env):
        ...
        # cleans nothing, since GetCaretBlinkTime takes no parameters).
