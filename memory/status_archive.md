...
**RESOLVED 2026-09-18 (see `status.md` / `TODO.md`):** the `screen.c(475)` / `ViewToScreen` `FMUL`-NaN mystery described below was a HOST x87 stack leak -- `_ = fpuPop(s);` discarded an `f80` return value, which Zig leaves on the host x87 stack, leaking one entry per popping FPU handler until a later host `fld` overflowed into a QNaN. Fixed with `fpuDrop()`. Every 'provably identical state' observation below is correct: the corruption was in host x87 state tew never inspected.
