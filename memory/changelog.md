...
**The `EIP=0x1901d9eb` crash pattern recurred a second time**, this time via `CompareStringA(locale=0x00000009)` — `MAKELANGID(LANG_ENGLISH, SUBLANG_NEUTRAL)`, a real, legitimate LCID. This is the third distinct locale value the old allowlist-based `_locale_is_valid` wrongly rejected (`0x0400`/`0x0800` in an earlier session, `LOCALE_INVARIANT` `0x007F` in x41, now `0x0009`). First pass removed `_locale_is_valid`/`_resolve_locale`/`_RESOLVABLE_LOCALES` entirely, accepting every locale unconditionally — **caught before merging**: `test_kernel32_io_compare_string.py`'s own module docstring documents the real original incident (2026-08-22, this file, further down) this validation exists for — `msjet35.dll`'s default-collating-order fallback (`FUN_7a84c830`) deliberately probes `CompareStringA(locale=0, ...)` to detect "no locale specified," and real (NT4/2000/XP-era) Windows genuinely rejects LCID `0` via `IsValidLocale`. Accepting *everything* would have silently reintroduced that exact original bug. **Corrected fix**: reject `locale == 0` only; every other value (including all three previously-rejected real LCIDs) compares unconditionally. Added `_log_locale_once` (shared, `debug` level) so each distinct non-en-US, nonzero locale is still recorded once per run for future reference, without the spam a per-call log would cause (confirmed live: some values fire 70+ times in under a second). Test suite updated to match: `test_unrelated_nonzero_locale_also_fails` (a real de-DE LCID) was itself part of the original over-correction, renamed to `..._succeeds` and inverted.

**Confirmed live**: with both fixes in place, execution runs further than any prior session, and `dblog.txt` (mcity's own DB thread's trace output — not Jet/DAO's directly, though its content is real `dbcode.c` source lines from the shipped Jet 3.5 engine) shows real database records actually coming back — the first time this has happened in tew. This effectively closes out the `EIP=0x1901d9eb` fault family and the broader multi-session DAO/Jet query-execution investigation.
...
**Test coverage added retroactively**: none of this session's new code (`CreateFileMappingA`/`MapViewOfFile`/`UnmapViewOfFile`, `fputs`, `_wcsicmp`, the crash-log JSON writer) shipped with any tests — caught when asked directly "how's coverage." Added `test_kernel32_io_file_mapping.py`, `test_msvcrt_fputs.py`, `test_msvcrt_wcsicmp.py`, and `test_exception_diagnostics.py` (38 new tests, all passing). Skipped: the CRT report-hook forwarding (`_crt_dbg_report` → guest callback) — needs a real CPU/scheduler for the reentrant call, higher effort than warranted right now. Also fixed two unrelated stale tests surfaced by a full-suite run: `test_kernel32_system_info.py`'s `GetEnvironmentStrings(A/W)` tests hardcoded a pre-fix address (superseded by real heap allocation, `commit 4d4d8e1`) and used a too-small memory fixture for the new heap-region address; `test_char_type.py`'s null-terminated-empty-string test still asserted the pre-fix (`c0bb64e`, x40) behavior instead of the intentional `CNTRL`-flag output. Confirmed both were pre-existing (fail identically on `main`, unrelated to this session) before fixing.
...
**Traced live, backward from the crash** (Ghidra never auto-recognized any of the functions in this chain — all created manually via `create_function`): `FUN_0f9dd9a7` (crash site) received its bad `param_2` unchanged from `FUN_0f9dd3d9`'s own `param_3`. `FUN_0f9dd3d9` is a private C++ vtable method (slot `+0x24`) on a session singleton constructed once by `FUN_0f9ddd11` (`_DAT_0fa10044`); a dynamic single-slot watchpoint (armed from the constructor's own entry probe, since the object's address is only known at runtime) confirmed that singleton's vtable is written correctly exactly once and never clobbered — ruling it out as the crashing object. The real caller, found via the crash's own EBP chain (frame[1], `MSJET35.DLL+0x61d87`), is `FUN_7a8a1c78` in msjet35.dll, whose vtable call passes a value called `uVar6` as the crashing argument. Reading `uVar6`'s source args via `[EBP+...]` initially returned garbage — `FUN_7a8a1c78`'s first instruction is a plain `MOV`, not `PUSH EBP`, so it never establishes that frame; fixed by capturing args at the function's true entry via `ESP` (correct regardless of whether the callee ever uses `EBP`). That confirmed the real value: `uVar6 == 0xFFFFFFFF`, an invalid-handle sentinel, not a garbage/miscomputed pointer.

**Root cause**: `kernel32.dll!CompareStringA(locale=0x0000007f)` was logging `-> 0 (invalid locale)` throughout the run, at `debug` level (easy to miss — flagged by Molly from one pasted log line, correctly refusing to accept it as more of the already-known-normal invalid-locale noise). `0x007F` is `LOCALE_INVARIANT`, a real, legitimate Windows LCID for culture-invariant comparisons — `_locale_is_valid` (`kernel32_io.py`) only ever accepted `0x0409` post-resolution, so every `LOCALE_INVARIANT` call was rejected, `EAX=0`. This is the same class of bug `_RESOLVABLE_LOCALES`'s own comments already document happening once before with `LOCALE_USER_DEFAULT`/`LOCALE_SYSTEM_DEFAULT` (that one silently turned every string comparison into "equal"). Something downstream (likely msjet35.dll's default-collation/parameter-table lookup, exact call site not yet pinned) reads the failure as "no match" and falls back to the `0xFFFFFFFF` sentinel that eventually reaches `FUN_7a8a1c78`.

**Fixed**: added `0x007F: 0x0409` to `_RESOLVABLE_LOCALES` (`kernel32_io.py`), same pattern as the existing entries. Bumped both `CompareStringA`/`CompareStringW` "invalid locale" log lines from `debug` to `error` — this exact code path has now caused two real, hard-to-spot bugs; it shouldn't be able to hide at debug level again. Confirmed live: the `0x1901d9eb` fault is completely gone.

**Immediately downstream, two new (unrelated) gaps found and one fixed**: `msvcrt.dll!_wcsicmp` was genuinely unimplemented (not a logic bug) — added a real cdecl implementation to `msvcrt_handlers.py` (mirrors the existing `wcsncmp`, reuses `read_wide_string` + `.upper()`). Confirmed live: execution runs past it too. Next new halt after that, not yet fixed: `kernel32.dll!CreateFileMappingA`, also genuinely unimplemented.

**Also shipped this session, unrelated to the fault chain above**:
- **Structured crash-log file**: `diagnose_fault`/`diagnose_halt` (`tew/kernel/exception_diagnostics.py`) now write the full DLL table + register/stack/EBP-chain dump to `/tmp/emu_crash.json` (overwritten every run) instead of inline into `/tmp/emu.log`. New `tools/crashlog_reader.py` reads it back and can resolve arbitrary ad-hoc addresses against that crash's own DLL table + static memory map, reusing `_annotate_address`'s real logic (refactored into `_classify_static_region` + a `_MEMORY_REGIONS` table) instead of re-implementing address ranges. Confirmed `DAO350.DLL` loads at `0x04470000` this run — inside what the old hardcoded "heap" literal range would have mis-labeled (harmless only because the DLL-table lookup runs first).
- **CRT report-hook forwarding**: `_crt_dbg_report` (`patch_internals.py`) now reads `__pfnReportHook` (`0x020ee23c`, confirmed via disassembly of the real, unpatched `_CrtSetReportHook`) and forwards every `_CRT_WARN` message to the guest's registered hook via `_invoke_emulated_proc`, scoped to `_CRT_WARN` only (`_CRT_ERROR`/`_CRT_ASSERT` still halt as before) — this is what would route the CRT's own memory-leak dump into `HookCrtLibraryErrorReports`'s `memleaksCRT.txt` on disk instead of only tew's debug log. Surfaced a real, separate gap it depends on: `msvcrt.dll!fputs` was entirely unimplemented (only `fwrite` existed) — added. Not yet exercised live (no run this session reached a clean process exit).

---

## 2026-08-29 (cont'd x40) — RESOLVED end to end: `StockAssembly_SelectAPT` (and every date-literal query) now succeeds. Real root cause was `classify_wide_string`/`GetStringTypeExW` leaving its output buffer unwritten when classifying a lone null terminator, letting a date-string tokenizer read stale leftover data and overshoot the end of a number by 2 WCHARs

**Context**: x39 fixed `GetLocaleInfoW` and found every locale/calendar-table gap it exposed (`_wtoi`, `_itoa`, a mislabeled `LOCALE_SMONTHNAME1..13` table entry, `GetCalendarInfoW`, `NlsGetCacheUpdateCount`, and a silent-stub gap in `MultiByteToWideChar`/`WideCharToMultiByte`'s logging — all fixed this session, each confirmed live before moving to the next). With all of that closed, `VarDateFromStr("1/1/2010")` still failed with `DISP_E_TYPEMISMATCH` — a *different* bug from x39's, exposed only once the locale struct was fully correct.

**Root cause, traced live through a chain of Ghidra-decompile-defeating VC6 functions** (`FUN_7716d2ff`, `FUN_7716bde4`, `FUN_7713cf60` are all declared `void` by Ghidra — same frame-pointer-omission blind spot as `FUN_7713cee1`/`FUN_77121505` in x39 — each one's real `EAX` return value had to be confirmed via a live entry/return probe rather than trusted from decompile): `VarDateFromStr`'s tokenizer classifies the end of a digit run via `FUN_7713cf60` → `FUN_7713cfd6` → real `GetStringTypeExW` → tew's `classify_wide_string` (`char_type.py`). That classification is done one character at a time via a tiny `[char, 0]` null-terminated buffer. When the character being classified is itself `0x0000` (i.e., testing the string's own null terminator, which is exactly how the tokenizer detects "end of number"), the buffer's effective length is `0`, so `classify_wide_string`'s loop ran zero iterations and left the output flags word completely unwritten. The caller read back stale leftover data instead — in this trace, the `DIGIT` flag from classifying the real digit immediately before it — so the null terminator (and a second stray null right after it, heap padding) both looked like "still a digit" and got consumed, overshooting the true end of the `"1/1/2010"` string by 2 WCHARs before finally hitting real garbage that correctly stopped the loop. That corrupted position pointer meant the *next* stage (checking what follows the number) saw garbage instead of end-of-string, the year token never got classified, and `VarDateFromStr`'s state machine (`switch(local_8)` at `0x7716dd7d`, went `0→3→5→0xd`) landed on state `0xd`, which has no case and falls straight to the function's terminal-failure return.

**Fixed**: `classify_wide_string` (`tew/api/char_type.py`) now special-cases a null-terminated (`cch_src=0xFFFFFFFF`) request whose computed length is `0` — it writes a real classification word for the terminator itself (`classify_ctype1(0)`, correctly `CNTRL` only) instead of leaving the output untouched. Confirmed live: the tokenizer's position now lands exactly on the real null terminator for the year token, the "end of string" classification correctly fires, and `VarDateFromStr` returns `EAX=0x0` for `"1/1/2010"`.

**Confirmed end to end**: `StockAssembly_SelectAPT`'s `"could not get param count"` error no longer appears anywhere in `stdout.txt`. The game now runs straight past the entire query into new territory, hitting a genuinely different, unrelated halt (`EIP=0x1901d9eb`, unhandled SEH fault) — a new frontier for next session, not a symptom of anything fixed here.

**Also resolved this session**: Molly's `FUN_77121505` "thread splat" suspicion (flagged in x39) — reran with `thread` logging enabled and it never fires. What actually fires is the already-documented, non-fatal `THREAD_SENTINEL` collision (`TODO.md`, "NEW (2026-08-26)") in `OLEAUT32.dll`'s `DllMain` static initializer — almost certainly what was originally observed. `FUN_77121505` itself is confirmed, separately, to be a correct stack-cookie check.

**Also fixed, unrelated**: `dll_loader.py`'s `patch_dll_iats` summary line undercounted "patched with stubs" by excluding auto-generated fatal-halt stubs from the numerator despite them being real stubs too (harmless only when that count was 0); now sums both categories and logs per-DLL instead of one aggregate line that could blur which of several DLLs loaded in one batch had a gap.

**Investigation method note**: this session repeatedly hit the same class of Ghidra limitation — VC6-compiled functions with no frame pointer (`SUB ESP, N` instead of `PUSH EBP; MOV EBP,ESP`) plus `stdcall` self-cleanup (`RET N`) reliably defeat Ghidra's return-value tracking, showing as `void` even when the function genuinely passes a value through `EAX`. The fix each time was the same: read the real register/stack state live at the call's entry and return address rather than trust the decompile's inferred `iVarN = FUN_xxx(...)` comparisons. This pattern accounted for essentially the entire second half of this session's investigation (`FUN_7716d2ff`, `FUN_7716bde4`, `FUN_7713cf60`).

---

## 2026-08-28 (cont'd x39) — Root cause found and fixed: `kernel32.dll!GetLocaleInfoW` was a bare "always fail, zero logging" stub, silently poisoning oleaut32's global locale-info cache for the whole process; `StockAssembly_SelectAPT` (and any other date-literal query) failed because of it, not a Jet SQL-compiler bug

**Root cause**: `GetLocaleInfoW` had never been implemented -- it unconditionally returned `EAX=0` (failure) with no logging whatsoever. Real `oleaut32.dll` code (`FUN_7713cee1`, reached from `VarDateFromStr`'s locale-struct-population chain, `FUN_77145656` -> `FUN_771454bc` -> `FUN_7713cee1`) calls this directly and unconditionally on its Wide code path. Its honest-but-silent failure meant `FUN_77145656` bailed out on its very first real locale query (`LOCALE_IDATE`), before ever setting the struct's own LCID field, calendar type, date separator, or anything else -- all left at zero-initialized allocation defaults. `FUN_77145563` (oleaut32's locale-struct cache manager) then unconditionally caches this broken struct and reports success regardless -- **confirmed via raw assembly, not just decompile**, that there is genuinely no `CMP`/`TEST` anywhere on the builder's return value between the call and the cache-list write. That single broken struct then gets served globally, to every thread, for every subsequent locale-info request, for the rest of the process's life. `VarDateFromStr` consulting this struct got a real but wrong date-order value (coincidentally `0`, matching M/D/Y by chance), an empty date separator, and an unclamped calendar type of `0` -- enough to send its tokenizer's state machine into a dead state before it ever got to parsing the year, producing `DISP_E_TYPEMISMATCH` for a perfectly valid `"1/1/2010"` literal.

**Why "zero log lines" looked like "never called" earlier in the same investigation**: `FUN_7713cee1` genuinely is called (confirmed via a direct entry-address probe, twice, with the correct `lcid=0x409`) -- its own real API call underneath was just silent. A `void`-declared Ghidra decompile of `FUN_7713cee1` also obscured that it reliably passes through `GetLocaleInfoW`'s real return value via `EAX`, because its cleanup call (`FUN_77121505`) is a hand-crafted `__chkesp`-style helper that transparently preserves whatever's already in `EAX` when there's no error -- Ghidra doesn't model that, so it reads the function as having no return value at all.

**Fixed**: implemented `GetLocaleInfoW` for real, moved into `kernel32_locale.py` next to `_get_locale_info_a` so both share the same `_LOCALE_STRINGS`/`_LOCALE_NUMBERS` tables (writes UTF-16 instead of ANSI bytes, `cch_data` counted in WCHARs). Two more real bugs found and fixed while wiring it up: the LCTYPE mask only stripped `LOCALE_RETURN_NUMBER` (`0x20000000`), not the separate `LOCALE_NOUSEROVERRIDE` (`0x80000000`) flag real callers OR in (`0x80000021` was failing lookup for what should just be `0x21`); and several LCTYPEs real code queries *without* `LOCALE_RETURN_NUMBER` (e.g. `LOCALE_IDATE`) expect the plain digit **string** form even though tew only had them in the numeric table -- added a `_locale_string_for()` fallback returning `str(value)` instead of duplicating every numeric entry into both tables.

**Confirmed live, post-fix**: all 4 real `GetLocaleInfoW` calls in this chain now succeed with correct values (`LOCALE_IDATE='0'`, `LOCALE_IFIRSTDAYOFWEEK='6'`, `LOCALE_IFIRSTWEEKOFYEAR='0'`, `LOCALE_ICALENDARTYPE='1'`). Execution proceeds further than ever before and hits a brand-new, unrelated, trivial gap: `msvcrt.dll!_wtoi` unimplemented -- not the same class of bug, a straightforward wide-string-to-int conversion, next session should implement it and confirm `StockAssembly_SelectAPT` (and the game's broader steady-state query loop, independently confirmed via `showplan.out` to now run for 500+ iterations without hanging) fully succeeds.

**Also fixed, found along the way, not the root cause**: `TlsGetValue` (`kernel32_sync.py`) had no real per-thread isolation -- `_tls_set_value` wrote to both a shared `TEB_BASE`-relative slot and a real per-thread store, but `_tls_get_value` only ever read the shared slot, so any two threads sharing a TLS index would clobber each other. Fixed to read from the per-thread store.

**Investigation method note**: this was found by live-tracing raw x86 execution through real, unmodified Microsoft DLL code (`msjet35.dll`'s expression parser, `oleaut32.dll`'s `VarDateFromStr` and its entire locale-info-cache machinery) using `cpu_add_logpoint`/`TEW_WATCH_ADDR`, cross-checked against Ghidra decompiles and, where the decompile's own accuracy was in doubt (Ghidra's known weakness around VC6-era disconnected tail blocks and non-standard calling conventions), raw instruction bytes -- not guessed from symptoms. Two prior sessions' worth of *actual* Jet SQL-compiler internals (`FUN_7a862215`, `FUN_7a85e7e1`, and neighbors) turned out to be entirely correct and irrelevant; the bug was several layers further down, in tew's own Win32 API surface, exactly matching this project's standing assumption that the bug is always tew's.
