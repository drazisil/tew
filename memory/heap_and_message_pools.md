...
- **One single 42MB block** (`44,040,192` bytes, allocation `#522`) — no file/line attribution at all (went through a plain `malloc()`, not the debug-instrumented `operator new` — matches `_MEM_initsize`'s own plain `malloc(param_1)` call from §1, though the size doesn't match `_MEM_init`'s hardcoded `0x4000000`/64MB constant exactly, so this may be a different large one-shot buffer, not confirmed which). This is **96% of all leaked bytes in one allocation** — very plausibly a legitimate, still-alive, by-design allocation rather than a bug: it shows up as "still allocated" simply because the process never reached a natural exit, the same way it would for any genuinely long-lived singleton or engine arena.
...
**Not yet done**: decompiling `dbcode.c(4024)`'s actual call site to confirm what it allocates and why nothing ever frees it (the clearest, most actionable next step); confirming whether the 42MB unattributed block really is `_MEM_init`'s arena despite the size mismatch, or something else.
